...
// String to show
let string = 'Happy<div>Birthday</div><div>Albee</div>'
if (xmas) string = 'Merry <div>Christmas</div>'
// let string = 'Merry <div>Christmas</div>'
...
function Flower([x, y]) {
	...
	// this.color = 260 + Math.random() * 30

	// 紅色
	// this.color = 0 + Math.random() * 5
	// this.color = 0
	// 紫色
	this.color = 270 + Math.random() * 10
	...
	particles.forEach((p) => {
		if (p.type === 0) {
			flowerInstancedMesh.setColorAt(
				flowerIdx,
				new THREE.Color('hsl(' + p.color + ', 80%, 75%)')
			)

			if (xmas) {
				flowerInstancedMesh.setColorAt(
					flowerIdx,
					// new THREE.Color('hsl(' + p.color + ', 100%, 75%)')
					// new THREE.Color('hsl(' + p.color + ', 80%, 75%)')
					// new THREE.Color('hsl(' + p.color + ', 80%, 50%)')
					new THREE.Color('rgb(255,0,0)')
					// new THREE.Color('hsl(265, 100%, 75%)')
					// new THREE.Color('hsl(0, 100%, 75%)')
				)
			}
